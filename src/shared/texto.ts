...
/** Nota de semelhança entre nomes, combinando letras e palavras distintivas. */
...
/** Lê um limite simples e ignora promoções do tipo "na segunda unidade". */
export function lerLimite(valor: unknown): number | null {
  if (valor == null) return null;
  const texto = String(valor).trim();
  if (!texto || /segunda unidade|segunda un|na segunda/.test(normalizarTexto(texto))) return null;
