...
function acharLinhaDoCabecalho(matriz: unknown[][]): number {
  for (let i = 0; i < Math.min(matriz.length, 25); i++) {
    const nomes = (matriz[i] ?? []).map((c) => normalizarTexto(String(c ?? "")));
    if (nomes.some((n) => /^(produto|nome|nome do produto|descricao|mercadoria)$/.test(n)))
      return i;
  }
  for (let i = 0; i < Math.min(matriz.length, 25); i++) {
...
export async function lerPlanilha(
  arquivo: File,
  opcoes: { preservarColunaA?: boolean } = {},
  ...
  const indicesAtivos = colunas
    .map((_, indice) => indice)
    .filter((indice) => !(ehCsv && !opcoes.preservarColunaA && indice === 0));
  return matriz
    .slice(cabecalho + 1)
    .filter((linha) => linha.some(temTexto))
    .map((linha) => {
      const registro: LinhaPlanilha = {};
      indicesAtivos.forEach((indice) => {
        registro[colunas[indice]!] = linha[indice] ?? "";
      });
...
export function valorDoCampo(linha: LinhaPlanilha, nomesPossiveis: string[]): unknown {
  const campos = Object.entries(linha).map(
    ([chave, valor]) => [normalizarCabecalho(chave), valor] as const,
  );
  ...
  for (const nome of nomesPossiveis) {
    ...
    const parcial = campos.find(
      ([chave, valor]) => temTexto(valor) && (chave.includes(alvo) || alvo.includes(chave)),
    );
    if (parcial) return parcial[1];
  }
  const procurandoLimite = nomesPossiveis.some((nome) =>
    normalizarCabecalho(nome).includes("limite"),
  );
  if (!procurandoLimite) return "";

  const porCabecalho = campos.find(
    ([chave, valor]) =>
      temTexto(valor) &&
      /\b(limite|qtd limite|quantidade limite|qtd max|quantidade max|maximo por cliente|max por cliente|cliente|cpf)\b/.test(
        chave,
      ),
  );
  if (porCabecalho && valorPareceLimite(porCabecalho[1])) return porCabecalho[1];

  const porTexto = campos.find(([chave, valor]) => {
    if (/produto|nome|descricao|mercadoria|preco|valor|oferta|clube|ean|codigo/.test(chave))
      return false;
    const texto = normalizarTexto(String(valor ?? ""));
    return (
      /\b(limite|limite de|por cpf|por cliente|fardo|fardos|kg|und|unidade)\b/.test(texto) &&
      /\d/.test(texto)
    );
  });
  ...
  const numerico = campos.find(([chave, valor]) => {
    ...
    if (
      /preco|preço|valor|oferta|clube|promocional|ean|codigo|cod|produto|nome|descricao|descrição/.test(
        chave,
      )
    )
      return false;
...
export function gerarPlanilhaDoClube(
  ofertas: OfertaParaExportar[],
  opcoes: OpcoesExportacaoClube,
): ArrayBuffer {
...
export function exportarModeloDoClube(
  ofertas: OfertaParaExportar[],
  opcoes: OpcoesExportacaoClube,
  nomeArquivo = "modelo para o clube.xlsx",
) {
  const conteudo = gerarPlanilhaDoClube(ofertas, opcoes);
