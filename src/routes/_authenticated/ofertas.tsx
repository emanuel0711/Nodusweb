...
function BarraAcao({ campoArquivo, processando, ofertas, notaMinima, setNotaMinima, nomeArquivo, precisamRevisao, limparOfertas, setModalAberto, processar }: ReturnType<typeof useOfertas>) {
  return <>
    <div className="surface flex flex-wrap items-center gap-3 p-5">
      ...
      <Button variant="destructive" disabled={!ofertas.length} className="ml-auto" onClick={() => { if (confirm("Excluir a planilha carregada?")) limparOfertas(); }}><Trash2 className="size-4" /> Excluir planilha</Button>
...
function TabelaOfertas({ ofertas, notaMinima, alterar, alterarAgrupamento, setModalVisualizacao }: ReturnType<typeof useOfertas>) {
  function remover(index: number) {
    ...
    try {
      const salvo = JSON.parse(sessionStorage.getItem("ofertaflow:rascunho-ofertas") || "{}") as Record<string, unknown>;
      sessionStorage.setItem("ofertaflow:rascunho-ofertas", JSON.stringify({ ...salvo, ...atual, ofertas: ofertas.filter((_, i) => i !== index) }));
      window.location.reload();
    } catch { /* o hook continua protegendo o estado principal */ }
  }
  ...
  return <div className="surface mt-4 overflow-x-auto"><Table><TableHeader><TableRow>
    ...
    <TableCell><Button variant="ghost" size="icon" aria-label="Remover item" onPointerDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); remover(index); }}><Trash2 className="size-4" /></Button></TableCell>
...
function EmptyState() { return <div className="surface mt-4 p-10 text-center text-sm text-muted-foreground">Envie a planilha da semana para começar. O sistema fará o cruzamento com o catálogo salvo.</div>; }
...
function DialogExportacao({ modalAberto, setModalAberto, carrossel, setCarrossel, ativarEm, setAtivarEm, inativarEm, setInativarEm, exportar }: ReturnType<typeof useOfertas>) {
  return <Dialog open={modalAberto} onOpenChange={setModalAberto}><DialogContent><DialogHeader><DialogTitle>Configurar arquivo do Clube</DialogTitle><DialogDescription>Escolha o carrossel e defina o período da oferta. A ativação usa data e hora.</DialogDescription></DialogHeader>
    <div className="space-y-4 py-2"><label className="block text-sm font-medium">Carrossel<select value={carrossel} onChange={(e) => setCarrossel(e.target.value)} className="mt-1.5 flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm outline-none"><option value="">Selecione um carrossel</option>{CARROSSEIS.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
...
function Info({ label, value }: { label: string; value: React.ReactNode }) { return <div><span className="text-muted-foreground">{label}</span><p className="font-medium">{value}</p></div>; }
