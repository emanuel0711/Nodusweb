...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Entrar — VarejoFlow" },
      { name: "description", content: "Acesse o VarejoFlow e continue sua operação." },
      { property: "og:title", content: "Entrar no VarejoFlow" },
      { property: "og:description", content: "Acesse sua conta no VarejoFlow." },
    ],
  }),
  component: AuthPage,
});
    ...
    <div className="flex min-h-screen items-center justify-center bg-secondary px-4 py-12">
      <div className="w-full max-w-md">
        <Link to="/" className="mb-6 flex items-center justify-center gap-2 font-display text-xl font-semibold">
          <span className="flex size-9 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Package className="size-5" /></span>
          VarejoFlow
        </Link>
          ...
          <Tabs defaultValue="login">
            ...
            {(["login", "signup"] as const).map((mode) => (
              <TabsContent key={mode} value={mode} className="mt-6 space-y-4">
                <div className="space-y-2"><Label htmlFor={`${mode}-email`}>E-mail</Label><Input id={`${mode}-email`} type="email" autoComplete="email" maxLength={255} value={email} onChange={(event) => setEmail(event.target.value)} placeholder="voce@empresa.com.br" /></div>
