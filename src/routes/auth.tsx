...
function getAuthErrorMessage(error: unknown) {
  ...
  if (/apikey|api key|invalid jwt|jwt/i.test(message))
    return "A chave de acesso foi rejeitada. Verifique a configuração do projeto.";
  const suffix = [value.code, value.status ? `HTTP ${value.status}` : ""]
    .filter(Boolean)
    .join(" — ");
...
function AuthPage() {
  ...
  return (
    <main className="auth-page min-h-screen px-5 py-8 text-foreground md:px-8">
      <div className="auth-shell mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl overflow-hidden md:grid-cols-[1.05fr_.95fr]">
        <section className="auth-story hidden flex-col justify-between p-10 md:flex lg:p-14">
          <Link to="/" className="font-serif text-3xl tracking-tight">
            Nódus<span className="text-primary">.</span>
          </Link>
          <div className="max-w-lg">
            <p className="mb-4 text-xs font-medium uppercase tracking-[0.22em] text-background/50">
              Organização · Automação · Clareza
            </p>
            <h1 className="font-serif text-5xl leading-[1.02] tracking-tight lg:text-6xl">
              Tudo conectado. Menos trabalho manual.
            </h1>
            <p className="mt-6 max-w-md text-sm leading-7 text-background/65">
              Entre para continuar organizando seus produtos, ofertas e processos em um único lugar.
            </p>
          </div>
          <div className="flex items-center gap-2 text-xs text-background/50">
            <ShieldCheck className="size-4" /> Acesso protegido
          </div>
        </section>

        <section className="auth-form-panel flex items-center justify-center p-6 md:p-10 lg:p-14">
          <div className="w-full max-w-md">
            ...
            {recoveryMode ? (
              <div className="space-y-6">
                ...
                >
                  Atualizar senha <ArrowRight className="ml-1 size-4" />
                </Button>
              </div>
            ) : forgotMode ? (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-muted-foreground">
                    Informe o e-mail da sua conta. Você receberá um link seguro para criar uma nova
                    senha.
                  </p>
                </div>
                <div className="space-y-2">
              ...
              <>
                ...
                <Tabs defaultValue="login" className="auth-tabs">
                  <TabsList className="h-11 w-full rounded-xl bg-muted p-1">
                    <TabsTrigger
                      value="login"
                      className="h-9 flex-1 rounded-lg data-[state=active]:bg-card"
                    >
                      Entrar
                    </TabsTrigger>
                    <TabsTrigger
...
function LegalConsent({
  ...
  return (
    <label
      className={`flex cursor-pointer items-start gap-3 rounded-xl border border-border/70 bg-muted/20 ${compact ? "p-3" : "p-4"}`}
    >
      <input
        type="checkbox"
        checked={checked}
