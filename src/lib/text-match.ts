...
export function similarity(a: string, b: string): number {
  ...
  // token overlap bonus: helps with reordered words
...
export function bestMatch<T extends MatchCandidate>(
  ...
  for (const candidate of candidates) {
    const score =
      normalize(candidate.description) === normalizedQuery
        ? 1
        : similarity(normalizedQuery, candidate.description);
...
/** "Limite 3 por CPF" -> 3 ; "2 un" -> 2 ; "" -> null */
